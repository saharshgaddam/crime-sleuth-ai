...
import { ZoomIn, ZoomOut, RotateCcw, X, Loader, UploadCloud, MessageSquare, Microscope, BrainCircuit } from "lucide-react";
...
export default function ImageAnalysisPanel({ 
  ...
  useEffect(() => {
    ...
    checkApiConnection();
  }, [FLASK_API_URL, setConnectionError]);
  ...
  const checkExistingSummary = async (imageId: string) => {
    ...
    try {
      const existingSummary = await API.forensic.getImageSummary(caseId, imageId);
      if (existingSummary) {
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      if (!exists) {
        ...
        const result = await API.forensic.generateImageSummary(caseId, selectedImage.id, imageFile);
  ...
  const sanitizeMarkdown = (text: string) => {
    ...
    // Properly handle asterisks for bold formatting
    // Replace ** markers with appropriate HTML tags for bold text
    // Using a robust pattern to handle nested formatting
